# Kubernetes Base Layer Configuration

This guide configures IBM Verify Access containers to a "base layer" state in Kubernetes. At this stage, containers can bootstrap successfully but require additional configuration for production use.

## Prerequisites

### System Requirements
- Kubernetes cluster (minikube, GKE, EKS, or equivalent)
...
- Deploy Verify Identity Access containers using [`../ivia-minikube.yaml`](../ivia-minikube.yaml) or equivalent
...
- **Deployment file:** Containers deployed via [`../ivia-minikube.yaml`](../ivia-minikube.yaml)
- **Hostnames configured:**
  - Management interface: `lmi.iamlab.ibm.com`
  - Reverse proxy: `www.iamlab.ibm.com`
- **DNS/hosts file:** These hostnames resolve to your Kubernetes cluster
...
6. **Create reverse proxy instance** - Establishes `rp1` instance for request handling
...
3. **Review container logs for errors:**
   ```bash
   kubectl logs -l app=ivia-config
   ```
...
- **Solution:** Verify hostname resolution: `nslookup lmi.iamlab.ibm.com`
...
- [`base_layer.sh`](base_layer.sh) - Automated setup script
- [`../ivia-minikube.yaml`](../ivia-minikube.yaml) - Kubernetes deployment manifest
...
- [`../ivia-backup-kubernetes.sh`](../ivia-backup-kubernetes.sh) - Backup utility
- [`../ivia-restore-kubernetes.sh`](../ivia-restore-kubernetes.sh) - Restore utility
